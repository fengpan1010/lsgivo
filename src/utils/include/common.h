...
inline void vMat_to_descriptors(const vector<cv::Mat>& vecDescriptorMat, cv::Mat& descriptorMat)
{
  descriptorMat = cv::Mat(vecDescriptorMat.size(), 32, 0);
...
inline void reduceVector_Vec2(vector<Vec2> &v, vector<unsigned char> status)
{
    int j = 0;
...
inline void reduceVector_cvMat(vector<cv::Mat> &v, vector<unsigned char> status)
{
    int j = 0;
    for (int i = 0; i < int(v.size()); i++)
        if (status[i])
